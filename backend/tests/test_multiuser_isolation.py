...
def test_user_registration_and_login(client):
    ...
    assert reg_resp.status_code == 200  # nosec B101
    assert "access_token" in reg_resp.json()  # nosec B101
    assert reg_resp.json()["user"]["username"] == "newuser"  # nosec B101
    ...
    assert login_resp.status_code == 200  # nosec B101
    assert "access_token" in login_resp.json()  # nosec B101
    ...
    assert dup_resp.status_code == 400  # nosec B101
...
def test_file_and_scan_isolation(client, user_a_headers, user_b_headers):
    ...
    assert files_b["total_files"] == 0  # nosec B101
...
def test_report_and_download_isolation(client, user_a_headers, user_b_headers):
    ...
    assert reports_a["total_reports"] == 1  # nosec B101

    # User B lists reports -> 0 reports
    reports_b = client.get("/api/v1/reports/", headers=user_b_headers).json()
    assert reports_b["total_reports"] == 0  # nosec B101

    # User B attempts to view User A's JSON report -> 404 DENIED
    assert client.get(f"/api/v1/reports/view/{report_name}", headers=user_b_headers).status_code == 404  # nosec B101

    # User B attempts to download User A's JSON report -> 404 DENIED
    assert client.get(f"/api/v1/reports/json/{report_name}", headers=user_b_headers).status_code == 404  # nosec B101

    # User B attempts to download User A's TXT report -> 404 DENIED
    assert client.get(f"/api/v1/reports/text/{report_name}", headers=user_b_headers).status_code == 404  # nosec B101

    # User B attempts to download User A's PDF report -> 404 DENIED
    assert client.get(f"/api/v1/reports/pdf/{report_name}", headers=user_b_headers).status_code == 404  # nosec B101

    # User B attempts to delete User A's report -> 404 DENIED
    assert client.delete(f"/api/v1/reports/{report_name}", headers=user_b_headers).status_code == 404  # nosec B101

    # User A can view and download own report
    assert client.get(f"/api/v1/reports/json/{report_name}", headers=user_a_headers).status_code == 200  # nosec B101
...
def test_ai_features_isolation(client, user_a_headers, user_b_headers):
    ...
    assert client.post(f"/api/v1/ai/analyze/{scan_id_a}", headers=user_b_headers).status_code == 404  # nosec B101

    # User B attempts to view stored AI analyses for User A's scan -> 404 DENIED
    assert client.get(f"/api/v1/ai/analyze/{scan_id_a}", headers=user_b_headers).status_code == 404  # nosec B101

    # User B attempts to get AI report for User A's scan -> 404 DENIED
    assert client.get(f"/api/v1/ai/reports/{scan_id_a}", headers=user_b_headers).status_code == 404  # nosec B101

    # User B attempts to get risk score for User A's scan -> 404 DENIED
    assert client.get(f"/api/v1/ai/risk/score/{scan_id_a}", headers=user_b_headers).status_code == 404  # nosec B101

    # User A can generate AI report for own scan
    assert client.get(f"/api/v1/ai/reports/{scan_id_a}", headers=user_a_headers).status_code == 200  # nosec B101
...
def test_admin_rbac_and_user_management(client, auth_headers, user_a_headers):
    ...
    assert client.get("/api/v1/admin/users", headers=user_a_headers).status_code == 403  # nosec B101

    # Admin user calls admin users endpoint -> 200 PASS
    admin_users_resp = client.get("/api/v1/admin/users", headers=auth_headers)
    assert admin_users_resp.status_code == 200  # nosec B101
    user_list = admin_users_resp.json()
    assert len(user_list) >= 2  # nosec B101
    ...
    assert deact_resp.status_code == 200  # nosec B101
    assert deact_resp.json()["user"]["is_active"] is False  # nosec B101

    # Deactivated User A attempts to request API using existing token -> 401 UNAUTHORIZED
    assert client.get("/api/v1/files/", headers=user_a_headers).status_code == 401  # nosec B101
    ...
    assert login_b.status_code == 401  # nosec B101
