...
class TestRiskEngine:
    ...
    def test_risk_history_persisted(self, db_session, sample_scan):
        ...
        assert history[0]["scan_id"] == sample_scan.id

    def test_risk_level_mapping(self, db_session):
        engine = RiskEngine(db_session)
        assert engine._score_to_risk_level(95) == "Low"
        assert engine._score_to_risk_level(70) == "Medium"
        assert engine._score_to_risk_level(50) == "High"
        assert engine._score_to_risk_level(30) == "Critical"
