...
def test_report_paths_reject_traversal_and_wrong_extensions(filename):
    ...
    assert report_error.value.status_code == 400  # nosec B101

    with pytest.raises(Exception) as email_error:
        email_report_path(filename)
    assert email_error.value.status_code == 400  # nosec B101
...
def test_login_errors_do_not_disclose_account_existence(client):
    ...
    assert unknown_user.status_code == wrong_password.status_code == 401  # nosec B101
    assert unknown_user.json()["detail"] == wrong_password.json()["detail"]  # nosec B101
